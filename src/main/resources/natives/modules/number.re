...
// parseFloat is usually the same as parseDouble
func parseFloat(s: str) -> float:
    return parseDouble(s)

// Parse a long integer from a string (usually same as parseInt but bigger)
...
func parseByte(s: str) -> byte:
    val = parseInt(s)
    return byte(val & 0xFF)  // clamp to byte range

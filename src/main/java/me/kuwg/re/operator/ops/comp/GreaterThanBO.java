...
public class GreaterThanBO extends BinaryOperator {
    ...
    public BOResult compile(final BinaryOperatorContext c) {
        ...
        if (leftType instanceof StrBuiltinType && rightType instanceof StrBuiltinType) {
            ...
            c.cctx().emit(resReg + " = icmp sgt i32 " + lLen + ", " + rLen);
        ...
        if (resultType instanceof FloatBuiltinType || resultType instanceof DoubleBuiltinType) {
            c.cctx().emit(resReg + " = fcmp ogt " + resultType.getLLVMName() + " " + leftReg + ", " + rightReg);
        } else {
            c.cctx().emit(resReg + " = icmp sgt " + resultType.getLLVMName() + " " + leftReg + ", " + rightReg);
        }

...
public final class LessThanBO extends BinaryOperator {
    ...
    public BOResult compile(final BinaryOperatorContext c) {
        ...
        if (leftType instanceof StrBuiltinType && rightType instanceof StrBuiltinType) {
            ...
            c.cctx().emit(resReg + " = icmp slt i32 " + lLen + ", " + rLen);
        ...
        if (resultType instanceof FloatBuiltinType || resultType instanceof DoubleBuiltinType) {
            c.cctx().emit(resReg + " = fcmp olt " + resultType.getLLVMName() + " " + leftReg + ", " + rightReg);
        } else {
            c.cctx().emit(resReg + " = icmp slt " + resultType.getLLVMName() + " " + leftReg + ", " + rightReg);
        }
